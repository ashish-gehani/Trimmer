...
STATISTIC(NumCompletelyUnrolled2, "Number of loops completely unrolled");
STATISTIC(NumUnrolled2, "Number of loops unrolled (completely or otherwise)");

static cl::opt<bool>
UnrollRuntimeEpilog2("unroll-runtime-epilog2", cl::init(false), cl::Hidden,
                    cl::desc("Allow runtime unrolled loops to be unrolled "
                             "with epilog instead of prolog."));

/// Convert the instruction operands from referencing the current values into
/// those specified by VMap.
static inline void remapInstruction2(Instruction *I,
...
static BasicBlock *
foldBlockIntoPredecessor2(BasicBlock *BB, LoopInfo *LI, ScalarEvolution *SE,
...
static bool needToInsertPhisForLCSSA2(Loop *L, std::vector<BasicBlock *> Blocks,
                                     LoopInfo *LI) {
...
const Loop* addClonedBlockToLoopInfo2(BasicBlock *OriginalBB,
...
bool UnrollLoop2(Loop *L, unsigned Count, unsigned TripCount, bool Force,
  ...
  for (unsigned It = 1; It != Count; ++It) {
    ...
    for (LoopBlocksDFS::RPOIterator BB = BlockBegin; BB != BlockEnd; ++BB) {
      ...
      if (*BB == Header) {
        ...
        const Loop *OldLoop = addClonedBlockToLoopInfo2(*BB, New, LI, NewLoops);
        if (OldLoop) {
    ...
    for (BasicBlock *NewBlock : NewBlocks) {
      for (Instruction &I : *NewBlock) {
        ::remapInstruction2(&I, LastValueMap);
  ...
  for (BasicBlock *Latch : Latches) {
    ...
    if (Term->isUnconditional()) {
      ...
      if (BasicBlock *Fold =
              foldBlockIntoPredecessor2(Dest, LI, SE, ForgottenLoops, DT)) {
  ...
  NumCompletelyUnrolled2 += CompletelyUnroll;
  ++NumUnrolled2;
  ...
  if (PreserveLCSSA && OuterL && CompletelyUnroll && !NeedToFixLCSSA)
    NeedToFixLCSSA |= ::needToInsertPhisForLCSSA2(OuterL, UnrolledLoopBlocks, LI);
...
MDNode * GetUnrollMetadata2(MDNode *LoopID, StringRef Name) {
