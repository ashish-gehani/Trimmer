                           ...
                           struct OperationConfig *config)
{
  ...
  */
  ...
  if(!config->cacert &&
     !config->capath &&
     !config->insecure_ok) {
    printf("inside certificate thing\n");
    char *env;
  ...
  if(config->postfields) {
    printf("inside postfields thing\n");
...
CURLcode operate(struct GlobalConfig *config, int argc, argv_item_t argv[])
{
  ...
  if(!result) {
    ...
    int len = strlen(config->first->url_list->url);
    printf("url %d, flags %d\n", len, config->first->url_list->flags);
    ...
    if(res) {
      ...
      if(!result) {
        ...
        result = get_args(operation, count++);
        int len = strlen(operation->useragent);
        printf("%s, %d is the useragent\n", operation->useragent, len);
