...
struct LongShort {
  const char letter[10]; /* short name option */
  const char lname[500];  /* long name option */
                            ...
                            struct OperationConfig *config)
{
  ...
  if(('-' != flag[0]) ||
    ...
    for(j = 0; j < 190; j++) {
      unroll_loop(190);
      if(!strncmp(aliases[j].lname, word, fnam)) {
        longopt = TRUE;
        numhits++;
        if(!strcmp(aliases[j].lname, word)) {
  ...
  if(!longopt) {
    letter = (char)*parse;
    subletter='\0';
    printf("!longopt -> letter = %c, subletter = %d\n", letter, subletter);
  }
  else {
    letter = parse[0];
    subletter = parse[1];
  ...
  for(int index = 0; index < 1; index++) {
    ...
    *usedarg = FALSE; /* default is that we don't use the arg */
    ...
    case '*': /* options without a short option */
      ...
      default: /* the URL! */
      {
        ...
        if(!url)
          ...
          // printf("%s\n", url->url);
          url->url = malloc(sizeof(char) * 500);
          memcpy(url->url, nextarg, strlen(nextarg));
    ...
    case '#': /* --progress-bar */
      if(toggle) {
        printf("%d is the progress meter *********************\n", CURL_PROGRESS_BAR);
    ...
    */
    ...
    if(config->url_list)
      printf("flags %d\n", config->url_list->flags);   
    if(!(!longopt && !singleopt && *++parse && !*usedarg))
      break;
    unroll_loop(1);
  }
  if(config->url_list)
...
__attribute__((annotate("specializeArg")))
ParameterError parse_args(struct GlobalConfig *config, int oldArgc,
                          argv_item_t oldArgv[])
{
  int i;
  bool stillflags;
  ...
  memcpy(argv[1], "--limit-rate", 12);
  memcpy(argv[2], "1K", 2);
  memcpy(argv[3], "-O", 2);
  memcpy(argv[4], "-#", 2);
  memcpy(argv[5], "http://demo.astra-net.com/download/200.jpg", 42);

  for(i = 1, stillflags = TRUE; i < 10; i++) {
    unroll_loop(10);
    if(i >= argc)
      break; 
    if(result)
    ...
    if(stillflags && ('-' == argv[i][0])) {
      ...
      if(!strcmp("--", argv[i]))
        ...
        if(operation->url_list)
          printf("flags %d\n", operation->url_list->flags);        
      ...
      result = getparameter(urlT, argv[i], &used, config,
                            operation);
    }
  }
