...
static void export_ef_problem(const char *s) {
  ...
  if (ef_client_globals.efcode != EF_NO_ERROR) {
    ...
    //-- HACKING -- aux = yices_create_context(logic_code, arch, CTX_MODE_ONECHECK, false, false);
...
int yices_main(int argc, char *argv[]) {
 ...
 /*
  ...
  // arch = CTX_ARCH_BV;
  // arch = CTX_ARCH_IFW;          // integer floyd-warshall
  // arch = CTX_ARCH_RFW;          // real floyd-warshall
  // arch = CTX_ARCH_EGFUN;        // egraph+array/function theory
  // arch = CTX_ARCH_EGBV;         // egraph+bitvector solver
  // arch = CTX_ARCH_EGFUNSPLX;    // egraph+fun+simplex
  // arch = CTX_ARCH_EGFUNBV;      // egraph+fun+bitvector
  // arch = CTX_ARCH_EGSPLXBV;     // egraph+simplex+bitvector
  // arch = CTX_ARCH_EGFUNSPLXBV;  // all solvers (should be the default)

  // arch = CTX_ARCH_AUTO_IDL;     // either simplex or integer floyd-warshall
  // arch = CTX_ARCH_AUTO_RDL;     // either simplex or real floyd-warshall
