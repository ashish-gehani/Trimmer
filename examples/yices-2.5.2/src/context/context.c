...
static void init_solvers(context_t *ctx) {
  ...
  //NOTE: init_solvers called from init_context
  uint8_t solvers;
  smt_core_t *core;
  smt_mode_t cmode;
  egraph_t *egraph;
  ...
  solvers = arch_components[ctx->arch];
  solvers = arch_components[CTX_ARCH_EGFUNSPLXBV];
  ctx->egraph = NULL;
  ...
  printf("init_solvers \n");
  //NOTE: Elimination takes place here
...
void init_context(context_t *ctx, term_table_t *terms, smt_logic_t logic,
  ...
  printf("init_context\n");
  // NOTE: init_context called from yices_create_context
