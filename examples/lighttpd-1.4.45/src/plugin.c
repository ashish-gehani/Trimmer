...
/* NOTE-HASHIM:  Initializes memory for plugin construct */
...
/* NOTE-HASHIM - plugins added to the src variable */
...
/* NOTE-HASHIM - Contains the list of init pointers */
...
int plugins_load(server *srv) {
	...
	plugin *p;
	size_t i, j;       
	// NOTE-HASHIM: Need to fully unroll this loop as well
        // NOTE-HASHIM: Adding loop-unroll hint?
        // NOTE-HASHIM: Phase1 - replace with constant tripcount
	for (i = 0; i < srv->srvconf.modules->used; i++) {
	        //NOTE-HASHIM - bitcast needs to be properly handled
 		data_string *d = (data_string *)srv->srvconf.modules->data[i];
		char *module = d->value->ptr;

		for (j = 0; j < i; j++) {
			if (buffer_is_equal(d->value, ((data_string *) 
                                           srv->srvconf.modules->data[j])->value)) {
                ...
                // NOTE-HASHIM - a) add unroll hint b) modify loop exit check
                // NOTE-HASHIM - Phase1 - replace with constant tripcount
                // NOTE-HASHIM - Need to constant fold the load_function constants
                // NOTE-HASHIM - Unless load_functions is removed, no DCE possible 
		for (j = 0; load_functions[j].name; ++j) {
		  // NOTE -HASHIM: Given modules and load_funcs are constant, can evaluate
...
#undef PLUGIN_TO_SLOT
		...
		if (p->init) {
			...
			if (p->version != LIGHTTPD_VERSION_ID) {
				log_error_write(srv, __FILE__, __LINE__, "sb",
						"plugin-version doesn't match lighttpd-version for", p->name);2
				return HANDLER_ERROR;
			}
		} else {
			p->data = NULL;
