...
#undef TEXT_IPV6
 ...
 /* NOTE-HASHIM - Adding function to read-in module list */
 // NOTE-HASHIM : TODO: Add file reading + Test with File IO transform
 void read_config_modules(server *srv, char * config_file){

   srv->srvconf.modules->data[0]->key->ptr = "mod_compress";
   srv->srvconf.modules->data[1]->key->ptr = "mod_cgi";
   srv->srvconf.modules->used = 2;
 }


int main (int argc, char **argv) {
        /* NOTE-HASHIM - Important variable (srv) - used in plugin initialization */
	server *srv = NULL;
	int print_config = 0;
	int test_config = 0;
	int i_am_root;
	...
	if (buffer_string_is_empty(srv->config_storage[0]->document_root)) {
        ...
        // NOTE-HASHIM - Using constant config file name
        char * config_file = "config_file.txt";
        //NOTE-HASHIM - Adding function to read modules
	read_config_modules(srv, config_file);


        // NOTE-HASHIM - plugins_load loads the plugins 
	if (plugins_load(srv)) {
		log_error_write(srv, __FILE__, __LINE__, "s",
