...
int main(int argc, char **argv)
{
	...
	// }
	// memcpy(argv[0], "tcptraceroute", 100);
	// memcpy(argv[1], "-i", 100);
	// memcpy(argv[2], "lo", 100);
	// memcpy(argv[3], "-n", 100);
	// memcpy(argv[4], "-A", 100);
	// memcpy(argv[5], "-m", 100);
	// memcpy(argv[6], "30", 100);
	// memcpy(argv[7], "google.com", 100);
	...
	for(i = 1; i < 10; i++)
	{
		...
		unroll_loop(10);
		printf("%s long\n", argv[i]);
		if (CHECKLONG(long_options[0])) {
			printf("calling usage 1\n");
			usage();
		}

		if (CHECKLONG(long_options[1]))
			about();

		/* undocumented, for debugging only */
		if (CHECKLONG(long_options[2]))
		{
			o_nofilter = 1;
		...
		if (CHECKLONG(long_options[6]) ||
			CHECKLONG(long_options[7]))
		{
			o_trackport = 0;
		...
		*/
		...
		if (strcmp(argv[i], long_options[12]) == 0)
			break;

		if (argv[i][0] == '-' && argv[i][1] == '-')
		{
			fprintf(stderr, "Unknown command line argument: %s\n", argv[i]);
			printf("calling usage 2\n");
			usage();
		}
	}
