...
char o_cfg[100]     = "/etc/knockd.conf";
char o_pidfile[100] = "/var/run/knockd.pid";
char o_logfile[100] = "";
			...
			if(ptr == NULL) {
				...
				if(!strcmp(section, "options")) {
					if(!strcmp(key, "LOGFILE")) {
						strncpy(o_logfile, ptr, 100-1);
						o_logfile[100-1] = '\0';
						dprint("config: log file: %s\n", o_logfile);
					} else if(!strcmp(key, "PIDFILE")) {
						strncpy(o_pidfile, ptr, 100-1);
						o_pidfile[100-1] = '\0';
...
long get_next_one_time_sequence(opendoor_t *door)
{
	...
	while(fgets(line, 100, door->one_time_sequences_fd)) {
		trim(line);
		if(strlen(line) == 0 || line[0] == '#') {
			pos = ftell(door->one_time_sequences_fd);
			continue;
		...
		if(flagsmatch && ip->protocol == attempt->door->protocol[attempt->stage] &&
			...
			if(attempt->stage >= attempt->door->seqcount) {
				...
				if(attempt->door->start_command && strlen(attempt->door->start_command)) {
					...
					if(fork() == 0) {
						/* child */
						char parsed_start_cmd[100];
						char parsed_stop_cmd[100];
						size_t cmd_len = 0;
