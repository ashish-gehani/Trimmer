...
static Function *getCalledFunctionThroughAliasesAndCasts(CallSite &CS) {
  ...
  if (BI.getOpcode() == BinaryOperator::SDiv ||
    ...
    if (const ConstantInt *CI = dyn_cast<const ConstantInt>(BI.getOperand(1))) {
      ...
      if (CI->getType()->isIntegerTy()) {
        APInt bitwidth(shift.getBitWidth(), CI->getType()->getIntegerBitWidth(),
                       true);
...
ProfilerPass::ProfilerPass()
    : ModulePass(ID), DL(nullptr), TLIWrapper(nullptr), TLI(nullptr),
      TotalFuncs("TotalFuncs", "Number of functions"),
      TotalSpecFuncs("TotalSpecFuncs", "Number of specialized functions"),
      TotalBounceFuncs("TotalBounceFuncs",
                       "Number of bounced functions added by devirt"),
    ...
    for (auto it = scc_begin(&CG); !it.isAtEnd(); ++it) {
      ...
      for (CallGraphNode *cgn : scc) {
        if (cgn->getFunction() && !cgn->getFunction()->isDeclaration()) {
          funcs.push_back({cgn->getFunction(),
                           {cgn->getNumReferences(),
...
void ProfilerPass::printCounters(raw_ostream &O) {
  ...
  std::vector<Counter> cfg_counters{
      TotalFuncs,         TotalSpecFuncs,   TotalBounceFuncs,   TotalBlocks,
      TotalInsts,         TotalDirectCalls, TotalExternalCalls, TotalAsmCalls,
