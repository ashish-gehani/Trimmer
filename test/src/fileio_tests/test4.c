...
//test file open and read calls with buffer initially non-constant but after reading static data from file, it becomes constant.
...
int main(int argc, char ** argv) {
  char buffer[100];
  externalFunc(buffer);
  int fd = open("../data/configFile4.txt", O_RDONLY);
