...
// ht points to an array of pointers to struct type of size 10. ht[8] is passed as an argument to an external function in function initialize. Therefore ht[8] becomes non-constant and branch in branchNotPruned function is not pruned. Rest of the array is constant so the branch in branchPruned function is pruned.
...
void initialize(struct HighType** obj) {
    ...
    obj[8]->lti->y[579] = 97;
    externalFunc(obj[8]);
    memcpy(&obj[8]->lts->y[661], "hdpwwwwg", 8);
}
...
void branchNotPruned(struct HighType** obj) {
    ...
    obj[8]->lti->y[516] == 100 ||
    obj[8]->lti->y[579] == 97 ||
    !strcmp(&obj[8]->lts->y[661], "hdpwwwwg") ||
